    ...
    with st.spinner("⚡ กำลังคำนวณ Variance Penalty และ MTBO Z-Score..."):
        ...
        for pos_idx, (position_name, col) in enumerate(positions.items(), start=1):
            ...
            html_card = (
                ...
                '<div style="color: #D32F2F; font-weight: 800; font-size: 15px; margin-bottom: 8px; text-transform: uppercase; letter-spacing: 1px;">🚫 ดับเอกฉันท์ 7 ตัว (Consensus Top-7)</div>'
                f'<div style="font-size: 36px; font-weight: 900; color: #B71C1C; letter-spacing: 8px; text-shadow: 1px 1px 0px rgba(255,255,255,0.8);">{format_dead(dead_final)}</div>'
                '</div>'
                ...
                f'<span style="background: #E8F5E9; color: #2E7D32; padding: 6px 15px; border-radius: 20px; font-weight: 800; font-size: 15px; letter-spacing: 3px;">{format_dead(dead_stat)}</span>'
                '</div>'
                ...
                f'<span style="background: #FFF3E0; color: #E65100; padding: 6px 15px; border-radius: 20px; font-weight: 800; font-size: 15px; letter-spacing: 3px;">{format_dead(dead_day)}</span>'
                '</div>'
                '</div>'
                
                '<div style="background: #263238; border-radius: 10px; padding: 15px; color: #CFD8DC; font-family: sans-serif;">'
                f'<div style="text-align: center; color: #ffffff; font-weight: bold; font-size: 14px; margin-bottom: 12px; letter-spacing: 0.5px;">⚡ Walk-Forward Analysis (WF {result["bt_steps"]} งวด)</div>'
                ...
                f'<div style="font-size: 18px; color: #ffffff; font-weight: 900;">{(result["bt_ai"]*100):.0f}%</div>'
                f'<div style="font-size: 11px; color: #90A4AE; margin-top: 2px;">น้ำหนัก: {(result["w_ai"]*100):.0f}%</div>'
                '</div>'
                ...
                f'<div style="font-size: 18px; color: #ffffff; font-weight: 900;">{(result["bt_stat"]*100):.0f}%</div>'
                f'<div style="font-size: 11px; color: #90A4AE; margin-top: 2px;">น้ำหนัก: {(result["w_stat"]*100):.0f}%</div>'
                '</div>'
                '<div style="flex: 1;">'
                '<div style="color: #FFB74D; font-size: 12px; margin-bottom: 4px;">📅 ความแม่นวัน</div>'
                f'<div style="font-size: 18px; color: #ffffff; font-weight: 900;">{(result["bt_day"]*100):.0f}%</div>'
        ...
        if all(x in store_final_probs for x in ["hundred", "ten", "unit"]):
            ...
            with col_sum1:
                html_sum1 = (
                    ...
                    f'<div style="font-size:28px; font-weight:900; color:#B71C1C; margin-top:15px; letter-spacing:4px;">{format_dead(dead_top)}</div>'
                    '</div>'
                )
                st.markdown(html_sum1, unsafe_allow_html=True)
        ...
        if all(x in store_final_probs for x in ["bot_ten", "bot_unit"]):
            ...
            with col_sum2:
                html_sum2 = (
                    ...
                    f'<div style="font-size:28px; font-weight:900; color:#B71C1C; margin-top:15px; letter-spacing:4px;">{format_dead(dead_bot)}</div>'
                    '</div>'
                )
