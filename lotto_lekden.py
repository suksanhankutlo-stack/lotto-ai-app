# ============================================================
# 🚀 LOTTO AI ULTIMATE ENSEMBLE
# SEQUENTIAL DRAW-TO-DRAW EDITION
# ============================================================
#
# แนวคิด:
#   • ไม่จดจำ Model
#   • ไม่จดจำ Backtest Weight
#   • ไม่ใช้ Joblib Model Cache
#   • ไม่ใช้ Joblib Memory Cache
#   • ทุกครั้งที่กดวิเคราะห์ = คำนวณใหม่ทั้งหมด
#   • Historical Data ใช้เป็นข้อมูลประกอบงวดปัจจุบันเท่านั้น
#
# Ensemble:
#   1. AI Ensemble
#      - RandomForest
...
def fetch_and_clean_data(url):
    ...
    try:
        ...
        df = df.dropna(
            subset=["Result_3D", "Result_2D"]
        )

        df = df.drop_duplicates(
            subset=["Date", "Result_3D", "Result_2D"]
        )
...
def build_features(
    ...
    df_feat["WeekOfYear"] = (
        df_feat["Date"].dt.isocalendar().week.astype(int)
    )
    ...
    for pos in positions:
        ...
        for i in range(len(values)):
            ...
            if i == 0:
                ...
                if prev_value in last_seen:

                    skips[i] = (
                        i - last_seen[prev_value]
                    )
...
class EnsembleEngine:
    ...
    def __init__(
        ...
        self.pos_sys = PositionalEquation()

        self.freq_sys = FrequencyEngine()

        self.cond_sys = ConditionalSystem()

        self.st_sys = StateTransitionSystem()
        ...
        if (
            ...
            for i in range(bt_size):
                ...
                for idx, c in enumerate(
                    proxy_model.classes_
                ):

                    ai_res[int(c)] = probs[idx]

                if actual_val in (
                    np.argsort(ai_res)[::-1][:5]
                ):
            ...
            if steps_run <= 0:
                ...
                bt_msg = (
                    f"(Backtest {steps_run} งวด: "
                    f"AI {int(ai_hits / steps_run * 100)}% | "
                    f"Freq {int(fq_hits / steps_run * 100)}% | "
                    f"ST {int(st_hits / steps_run * 100)}% | "
                    f"Cal {int(cal_hits / steps_run * 100)}% | "
                    f"Pattern {int(ptn_hits / steps_run * 100)}%)"
                )
    ...
    def predict_all(self):
        ...
        # Dummy row
        #
        # ใช้เฉพาะสร้าง Calendar / Lag structure
        # ไม่ใช้ 000/00 เป็นข้อมูล Training
        # ----------------------------------------------------
...
st.title(
    "🚀 ระบบวิเคราะห์เลขเด่น Ultimate Ensemble"
)
...
st.caption(
    "🔄 ทุกครั้งที่วิเคราะห์จะคำนวณใหม่ทั้งหมด "
    "โดยไม่บันทึก Model / Weight / Prediction"
)
...
if st.button(
    "🚀 วิเคราะห์เลขเด่น V.Max Sequential",
    type="primary",
    use_container_width=True
):

    with st.spinner(
        "⏳ กำลังดึงข้อมูลและคำนวณใหม่ทั้งหมด..."
    ):
        ...
        weights_str = (
            ...
            f"RF={engine.ai_weights[0]} | "
            f"ET={engine.ai_weights[1]} | "
            f"HGB={engine.ai_weights[2]} | "
            f"XGB={engine.ai_weights[3]}"
        )
...
- 📊 Lags = **{engine.lags}**
- 📈 Rolling = **{engine.rolls}**
- 🤖 AI = **RF + ET + HGB + XGB**
- 🧠 Ensemble = **AI + Frequency + Calendar + ST + Pattern + Equation**
- 🔄 **Sequential: คำนวณใหม่ทุกครั้ง**
- 🚫 **ไม่มี Model Cache**
- 🚫 **ไม่มี Weight Cache**
- 🚫 **ไม่มี Prediction Memory**
"""
        )
        ...
        for pos in [
            ...
            with st.expander(
                ...
                st.markdown(
                    f"- 🤖 **เลขเด่น AI** : `{nums_ai}`"
                )

                st.markdown(
                    f"- 📅 **เลขเด่น กำลังวัน** : `{nums_day}`"
                )

                st.markdown(
                    f"- 📊 **เลขเด่น สถิติ** : `{nums_stat}`"
                )
        ...
        probs_top = (
            ...
            preds["H"]["Probs_For_Graph"]

            +

            preds["T"]["Probs_For_Graph"]

            +

            preds["O"]["Probs_For_Graph"]
        ...
        probs_bot = (
            ...
            preds["T2"]["Probs_For_Graph"]

            +

            preds["O2"]["Probs_For_Graph"]
...
st.caption(
    "⚠️ ระบบเป็นการวิเคราะห์เชิงสถิติและ Machine Learning "
    "ไม่สามารถรับประกันผลรางวัลได้"
)
